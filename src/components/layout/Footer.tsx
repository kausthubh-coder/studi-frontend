...
const navigation = {
  ...
  social: [
    {
      name: 'GitHub',
      href: 'https://github.com/your-organization/studi',
    ...
    <footer className="bg-white dark:bg-gray-800">
      <div className="container mx-auto overflow-hidden py-12 px-4 sm:px-6 lg:px-8">
        <nav className="-mx-5 -my-2 flex flex-wrap justify-center" aria-label="Footer">
          {navigation.main.map((item) => (
            <div key={item.name} className="px-5 py-2">
              <Link to={item.href} className="text-base text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">
                {item.name}
              </Link>
            </div>
          ))}
        ...
        <p className="mt-8 text-center text-base text-gray-500 dark:text-gray-400">
          &copy; {new Date().getFullYear()} Studi. All rights reserved.
        </p>
      </div>
    </footer>
  );
